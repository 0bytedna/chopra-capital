  ...
  return <div className="mx-auto max-w-7xl space-y-8">
    ...
    <section className="grid grid-cols-2 gap-4 lg:grid-cols-4" aria-label="Pool snapshot">
      {[
        ...
        { label: "Trading equity", value: `${formatUsdt(poolNav.equity)} USD`, hint: "includes open P/L" },
        { label: "NAV / unit", value: formatUsdt(poolNav.nav, 6), hint: "equity ÷ issued units" },
        { label: "In queue", value: `${formatUsdt(queuedTotal)} USD`, hint: "verified, awaiting investment" },
      ].map((item) => <div key={item.label} className="glass-card rounded-xl p-4 sm:p-5"><p className="text-[10px] uppercase tracking-[0.14em] text-ink-faint">{item.label}</p><p className="mt-1.5 font-mono text-lg text-ink">{item.value}</p><p className="mt-0.5 text-[11px] text-ink-faint">{item.hint}</p></div>)}
    </section>

    <section className="grid gap-5 xl:grid-cols-2">
      <AdminActionForm action={adminSetTradingSnapshot} submitLabel="Save balance & equity" pendingLabel="Saving…" variant="gold" className="glass-card rounded-2xl p-5 sm:p-6" confirmMessage="Replace the current trading balance and equity with these values?">
        <p className="eyebrow">Manual snapshot</p><h2 className="mt-2 font-serif text-xl text-ink">Set current balance and equity</h2>
        <div className="mt-4 grid gap-3 sm:grid-cols-2"><label className="space-y-1.5 text-xs text-ink-faint">Balance (USD)<input name="balance" type="number" min="0" step="0.01" defaultValue={poolNav.balance.toFixed(2)} required className="h-10 w-full rounded-lg border border-gold-600/20 bg-vault-950/65 px-3 text-sm text-ink" /></label><label className="space-y-1.5 text-xs text-ink-faint">Equity (USD)<input name="equity" type="number" min="0" step="0.01" defaultValue={poolNav.equity.toFixed(2)} required className="h-10 w-full rounded-lg border border-gold-600/20 bg-vault-950/65 px-3 text-sm text-ink" /></label></div>
