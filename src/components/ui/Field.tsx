...
export function Field({ label, name, hint, error, className, ...props }: FieldProps) {
  return (
    <div className={cn("space-y-1.5", className)}>
      <label htmlFor={name} className="block text-xs font-medium uppercase tracking-[0.14em] text-ink-dim">
        {label}
      </label>
...
export function TextareaField({ label, name, hint, error, className, ...props }: TextareaFieldProps) {
  return (
    <div className={cn("space-y-1.5", className)}>
      <label htmlFor={name} className="block text-xs font-medium uppercase tracking-[0.14em] text-ink-dim">
        {label}
      </label>
      <textarea id={name} name={name} rows={4} className={inputClass} {...props} />
      {hint && !error && <p className="text-xs text-ink-faint">{hint}</p>}
      {error && <p className="text-xs text-negative">{error}</p>}
    </div>
  );
